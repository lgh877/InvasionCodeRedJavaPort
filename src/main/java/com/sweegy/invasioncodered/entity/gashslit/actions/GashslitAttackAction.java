...
public class GashslitAttackAction{
    ...
    public static void execute(LevelAccessor world, GashslitEntity mob, double x, double y, double z) {
        ...
        if (actionTicks == 5) {
            ...
            if (target != null) {
                ...
                double flatDist = Math.min((double) diffVec.length(), width * 3);
            ...
            world.playSound(null, BlockPos.containing(hitboxPos.x(), hitboxPos.y(), hitboxPos.z()), InvasioncoderedsweegyportModSounds.GASH_ATTACK.get(), SoundSource.HOSTILE, 2, 1);
            ...
            target = mob.getTarget();
            if (target != null) {
                GashslitActionSelector.INSTANCE.evaluateAndChoose(mob, target, x, y, z);
            }
        }
    }
}
